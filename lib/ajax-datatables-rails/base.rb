    ...
    def as_json(options = {})
      {
        ...
        :recordsTotal => (@query_has_group_by) ? ((get_raw_records).length) : (get_raw_records.count(:all)),
        :recordsFiltered => (@query_has_group_by) ? (filter_records(get_raw_records).length) : (filter_records(get_raw_records).count(:all)),
        :data => data
      }
    end
